...
export class DataServiceListComponent implements OnInit {
  ...
  currentAmphueList = JSON.parse(JSON.stringify(this.amphueList));
  ...
  onOpenFile(a: any) {}
  ngOnInit(): void {
    this.formGroup.get('province')?.valueChanges.subscribe((v) => {
      this.formGroup.get('amphue')?.reset();
      if (v && v?.length > 0) {
        this.currentAmphueList = this.amphueList.filter((a) =>
          v.some((p) => p === a.provinceId)
        );
      } else {
        this.currentAmphueList = JSON.parse(JSON.stringify(this.amphueList));
      }
    });
